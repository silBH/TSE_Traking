 ...
 * traking.tools.Conexion; import org.postgis.Point; import
 * org.postgis.PGgeometry; import java.text.ParseException;
 * 
 * // Establecer la conexión con la base de datos Conexion dataConexion = new
 * Conexion(); Connection connection = dataConexion.getConnection();
 * 
 * if (connection != null) { PreparedStatement statement = null; ResultSet
 * resultSet = null;
 * 
 * try { // Crear la consulta SQL String query = "SELECT * FROM traking";
 * 
 * // Preparar el statement statement = connection.prepareStatement(query);
 * 
 * // Ejecutar la consulta resultSet = statement.executeQuery();
 * 
 * // Recorrer los resultados y crear instancias de TrackingModel while
 * (resultSet.next()) { Long id = resultSet.getLong("id"); String matricula =
 * resultSet.getString("matricula"); String pais = resultSet.getString("pais");
 * 
 ...
 * 
 * TrackingModel traking = new TrackingModel(id, matricula, pais, coordenadas,
 * date); listado.add(traking); } } catch (SQLException e) {
 * System.err.println("Error al ejecutar la consulta: " + e.getMessage()); }
 * catch (ParseException e) {
 * 
 * e.printStackTrace(); } finally { // Cerrar el resultSet, el statement y la
 * conexión try { if (resultSet != null) { resultSet.close(); } if (statement !=
 * null) { statement.close(); } dataConexion.closeConnection(); } catch
 * (SQLException e) { System.err.
 * println("Error al cerrar el resultSet, el statement o la conexión: " +
 * e.getMessage()); } } }
 */
